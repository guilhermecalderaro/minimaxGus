...
public class NewAgent2 implements PlayerAgent {
    ...
    public String setPiece(GameInfo gi) {
        MinimaxTree arvore = new MinimaxTree(4, MinimaxTree.SET_ACTION, gi) {
            @Override
    ...
    private int evaluate(int w1, int w2, int w3, int w4, int w5, int w6, MinimaxTree.Node node, GameInfo gi) {        
        ...
        int notaAvaliacao = 0;
        int[][] gs = node.getGameState();
        
        //Retorna total de linhas fechadas pelo jogador de vantagem
        if(w1 != 0){
            notaAvaliacao += (w1 * getTotalLinhasFechadas( MAX_TURN, gi, gs ) - (w1 * getTotalLinhasFechadas( MIN_TURN, gi, gs)));
        }
        
        //Retorna total de peças bloqueadas do jogador de vantagem
        if(w2 != 0){
            notaAvaliacao += (w2 * getTotalPecasAdversariaBloqueadas( MIN_TURN, gi, gs) - (w2 * getTotalPecasAdversariaBloqueadas( MAX_TURN, gi, gs)));
        }
        
        //Total de peças do jogador de vantagem
        if(w3 != 0){
            notaAvaliacao += (w3 * gi.getPlayerSpots(gs).size()) - (w3 * gi.getOpponentSpots(gs).size());
        }
        
        //Quantidade de vezes que o Jogador possui de L de duas peças de vantagem
        if(w4 != 0){
            notaAvaliacao += (w4 * getQuantidadeLDuasPecas( MAX_TURN, gi, gs) - (w4 * getQuantidadeLDuasPecas( MIN_TURN, gi, gs)));
        }
        
        //Quantidade de vezes que o Jogador possui de L de três peças de vantagem
        if(w5 !=0){
            notaAvaliacao += (w5 * getQuantidadeLTresPecas( MAX_TURN, gi, gs) - (w5 * getQuantidadeLTresPecas( MIN_TURN, gi, gs)));
        }
        
        //Quantidade de vezes que o Jogador possui de L de cinco peças de vantagem
        if(w6 !=0){
            notaAvaliacao += (w6 * getQuantidadeLCincoPecas( MAX_TURN, gi, gs) - (w6 * getQuantidadeLCincoPecas( MIN_TURN, gi, gs)));
        }
    ...
    public int getTotalLinhasFechadas(int jogador, GameInfo gi, int[][] gs){
        ...
        for (int i=0; i<playerSpots.size(); i++){
            ...
            */
            else if( coluna % 2 == 1  && linha == LINHA_ZERO){
    ...
    public void teste(GameInfo gi, int[][] gameState){
        ...
        int jogador = (2*getQuantidadeLDuasPecas(MAX_TURN, gi, gameState)) + (3*getQuantidadeLTresPecas(MAX_TURN, gi, gameState));
        int oponente = (2*getQuantidadeLDuasPecas(MIN_TURN, gi, gameState)) + (3*getQuantidadeLTresPecas(MIN_TURN, gi, gameState));
